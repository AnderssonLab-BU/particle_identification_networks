...
class AstigmatismDataset(Dataset):
    def __init__(self, data_cfg):
        super().__init__()
        self._logger = get_logger("astigmatism_dataset")
            ...
            for image in range(self.data_cfg.IMAGE_RANGE):
                ...
                for g in self.data_cfg.G_VALUES:
                    path_to_image = os.path.join(
                        self.data_cfg.HOME, "%dspots_img_N10G%d_astig" % (self.data_cfg.NUM_SPOTS, g),  "%dspots_img_N10G%dTrial%d" % (self.data_cfg.NUM_SPOTS, g, trial), "%04d.jpg" % image)
                    self.images.append(
